...
def create_model_and_export_excel(charging_speed, ghg_cost_per_tonne, x_chr_domain, locs):
    ...
    m.CRTP = Param(m.T, initialize=[tou_prices[t] for t in list(m.T)[:len(tou_prices)]])
    ...
    if x_chr_domain == NonNegativeReals:
        excel_file_name = f"4BEV_smart_{charging_speed}g_{ghg_cost_per_tonne}kw_{locs}.xlsx"
    else:
        excel_file_name = f"4BEV_v2g_{charging_speed}g_{ghg_cost_per_tonne}kw_{locs}.xlsx"
    ...
    for speed in charging_speeds:
        for cost in ghg_costs:
            for loc in locations:
                excel_file, json_file = create_model_and_export_excel(charging_speed=speed, ghg_cost_per_tonne=cost, x_chr_domain=domain, locs=loc)
                print(f" file '{excel_file}' has been created with charging speed {speed} kW and GHG cost ${cost} per tonne, V2G at {loc}.")
